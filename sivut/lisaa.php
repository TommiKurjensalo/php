...
if (isset($_POST["tallenna"])) {
   // Viedään muodostimelle kenttien arvot
	...
	if (!isset($_POST["asiakkaanNimi"]) ||
			...
			!isset($_POST["kayttoJarjestelmaId"]) ||
		...
		// Luodaan uusi olio, jolla lisätään asiakastiedot
			if ($syottoVirhe === FALSE) {
				try {
					...
					$rivit = $kantakasittely->lisaaAsiakas($lisaa);
	...
	elseif (isset($POST["peruuta"])) {
			...
			(isset($lisaa) ? $lisaa = New Lisaa():'');
			(isset($_SESSION["lisaa"]) ? $_SESSION["lisaa"] = array():'');
	...
	// Onko painettu tallenna painiketta ja onko sessiossa lisaa olio talletettu
		elseif (isset ( $_POST ["tallenna"] )) {
			if (isset ( $_SESSION ["lisaa"] )) {
				...
				// Jos sessiosta löytyy lisaa olio, joka ei ole tyhjä. annetaan se $lisaa oliolle
				if (isset($_SESSION["lisaa"]) && !empty($_SESSION["lisaa"])) {
		...
		} // if (isset ( $_SESSION ["lisaa"] )) -> else 
} // (isset ( $_POST ["tallenna"] -> else
                    ...
                    <div class="col-lg-12">
                        <h1 class="page-header">
                         ...
                         if (isset($_COOKIE["isDebug"])) {
                        	...
                        	"<br>".'sessio lisaa: '; echo (isset($_SESSION['lisaa']) ? var_dump($_SESSION['lisaa']) : '').
                        	"<br><br>".'lisaa: '; echo (isset($lisaa) ? var_dump($lisaa) : '');
                         }
			   			?>
                    ...
                    <div class="col-lg-6"> <!-- vasen col-lg-6 -->
                        ...
                        <form class="inline-form" role="form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
                           ...
                           <?php echo '<input name="asiakkaanNimi" class="form-control" type="text" value='.
                             (($syottoVirhe === TRUE) ? '"'.$lisaa->getAsiakkaanNimi().'"' 		
								...
								<div class="input-group">
		                            ...
		                            <?php 
									    if (!empty($rivit)) {
											...
											</i> Asiakas on lisätty onnistuneesti!</h3>'
													: '<h3 style="color:red"><i class="glyphicon glyphicon-remove">
													</i>Päivitys epäonnistui</h3>');
					    				}
		                            echo ((!empty($virheviesti) && $virheviesti != null) ? '<br><h3 style="color:red";>' .$virheviesti. '</h3>':'');
